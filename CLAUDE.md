...
Feature folders live under `src/features/` (`auth`, `home`, `settings`, `workspace`, `tutorial`), each with a `components/ model/ services/` shape (`tutorial` has no `services/` — it's local UI-only state, not backed by a server feature: a guided-tour spotlight, a per-page tutorial modal registry in `model/tutorial-content.ts`, and a trigger button, aimed at onboarding new users to the workspace UI). Every feature is built against a **service interface** injected from `App.tsx`, with both a mock and (where wired) a real HTTP implementation — that seam is what lets component tests run without network. `settings` and `workspace` are wired to the real server (`http-settings-service.ts`, `HttpWorkspaceService` with 2s polling); `home` still runs on a mock.
...
```
orquestador → calculo → sintesis (escritura → shell) → curador ⟲ → documentador → END
```

`orquestador` can also short-circuit straight to `END` without touching `calculo`/`sintesis`/`curador` at all — see `mode: chat`/`clarify` below.

- `orquestador` (`src/agents/orquestador/`) — accepts either `request_text` (natural language, resolved by an LLM) or a structured `circuit_spec` dict, validated against the Pydantic schema in `orquestador/schema.py`. Both paths normalize into sub-blocks with goals. When resolving `request_text`, the LLM's output is a discriminated `mode`: `chat` (not a design request — replies directly, graph ends), `clarify` (design intent but missing info — asks a `question`, carries `partial_spec`, graph ends), or `design` (the usual full `CircuitSpec`, continues to `calculo`). Only the `design` path is a "run" in the usual sense.
- `calculo` (`src/agents/calculo/`) — Master-Worker subgraph, one worker per sub-block via the Send API, closed-form component formulas per curated type (`calculo/formulas.py`). For `generic` blocks (see below) there is nothing to compute — the worker passes the LLM-authored netlist straight through `component_values` unchanged.
...
- `curador` (`src/agents/curador/`) — a reward-driven accept/adjust/reject policy (`curador/reward.py`, `curador/policy.py`), not the "deterministic rules only" placeholder described previously: `R = -Σᵢ(wᵢ·APEᵢ) + β·c - γ·n` over each iteration; `choose_action` picks `accept` vs `adjust` by comparing the accept reward against the projected reward of one more adjustment (estimated via the observed error-reduction ratio ρ from `history`), with `accept_is_admissible` as a hard guardrail so a stalled-but-rewarded circuit outside its own tolerance is never accepted. `reject` fires only when iterations run out. This is the reward-shaping half of the thesis's RL design — there is still no learned/trained policy, the reward formula is fixed and evaluated directly. Closes the loop back to `sintesis` on adjust, bounded by `max_iterations` (default 5). For `generic` blocks, adjustment means asking the curador's own LLM to repair the netlist (`curador/reparacion.py`) rather than perturbing a formula's inputs; two repairs that produce an unchanged netlist give up on that block rather than burn the remaining iterations.
- `documentador` (`src/agents/documentador/`) — runs once after `curador` accepts, LLM-summarizes each block's final netlist (components, tags, a plain-language explanation of the measurement). Never affects the verdict: any failure (no LLM configured, extraction error) just leaves that block's `documentation` as `None`.

Invalid specs and exhausted iterations end at `END` with a populated `verdict` rather than raising — the graph always terminates. Curated circuit types: `voltage_divider`, `rc_lowpass`, `led_resistor`, `noninverting_amp`. There is also a `generic` block type (`orquestador/schema.py`'s `GenericBlock`) for anything outside that catalog: the LLM authors a complete SPICE netlist (with a `.control` block and an `output.txt` write, validated by a Pydantic check) instead of a closed-form formula. The premise isn't that the generated netlist is trustworthy — it's that `ngspice`/`curador` never assume it is; the netlist is simulated and judged exactly like a curated block's, including going through the same accept/adjust/reject loop.

Compiled with an injectable checkpointer (`checkpointer` param of `build_graph()`). `src/agents/checkpointer.py`'s `open_checkpointer()` picks `PostgresSaver` against the same Neon database (via `CHECKPOINTER_URL`, optionally with a `search_path` schema so LangGraph's own migrations don't collide with Drizzle's) when that env var is set, and falls back to in-memory `MemorySaver` otherwise — tests and local dev don't need a database; only resuming an interrupted run does. One-time setup: `uv run --env-file .env python -m agents.checkpointer`.

**Evaluation harness** (`src/agents/evaluacion/`) — separate from the pipeline itself, built for the thesis's experimental chapter. `evaluacion/banco.yaml` (data, not code) holds test cases (`id`, `descripcion`, `spec`, a `referencia` with expected metric/target/components); `corredor.py` runs the graph over each case, `metricas.py` summarizes, `reporte.py` renders a markdown or LaTeX table. Run with `uv run python -m agents.evaluacion` (`--banco <path>` for a custom bench, `--latex` for the LaTeX table).

**LLM resolution.** Four nodes consume an LLM: `orquestador` (interpreting `request_text` and choosing chat/clarify/design), `curador` (repairing a `generic` block's netlist on adjust — the curated-type adjustment math itself stays deterministic), and `documentador` (post-hoc netlist summary). `calculo`'s closed-form formulas and `sintesis`/`shell` (running `ngspice`) stay fully deterministic regardless of block type. The LLM is never configured inside agents: `src/agents/llm/settings_client.py` fetches it from the server per agent via `GET /api/internal/llm/agent/:agentId?userId=`, cached 60s in memory keyed by `(agent_id, user_id)`. Requires `SERVER_BASE_URL` and `AGENTS_SERVICE_TOKEN`; without them the `request_text` path is unavailable but the structured `circuit_spec` path keeps working (a `generic` block still needs the curador's LLM to be configured if it ever needs repair).
...
- `src/app.ts` — builds the app via `createApp()`, calls `configureOpenAPI(app)`, chains the module routers (currently `authRouter`, `llmRouter`, `workspaceRouter` and `healthRouter`) and exports `AppType`, the type the client's RPC client consumes.
...
**Existing modules:** `auth`, `llm`, `workspace` and `health` (`GET /api/health`, trivial liveness check, no auth).
...
`workspace.runner.ts`'s `resolveRunOutcome` maps the agents pipeline's result into what gets persisted, distinguishing the `chat`/`clarify` outcomes (see the agents `orquestador` above) from an actual design verdict: for `chat` and `clarify` turns it sets `artifacts: null` and only updates the summary/assistant message (and, for `clarify`, `normalizedSpec` from `outcome.partial_spec`); for the design path it falls back to `mapVerdictToStatus(verdict)` plus a real `toArtifactDrafts(result)` array. `makeDbSink` in `workspace.services.ts` only deletes/re-inserts artifacts when `outcome.artifacts !== null` — so a chat/clarify turn never wipes the conversation's existing artifacts, it just skips touching them.
